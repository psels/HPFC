...
def build_local_time_effect(    df,    resid_col="resid_after_daytype",    date_col="Datum",    day_type_col="day_type",    hour_col="hour",    window_days=45,    year_length=366,):
    """
    for every combination (day_type, hour, day_year),
    compute mediane of resid_col for observations :
    - same day_type
    - same hour
    - in a circular window of +/- around dayofyear
...
def build_local_residual_quantiles(
    df: pd.DataFrame,
    resid_col: str = "final_residual",
    day_type_col: str = "day_type",
    hour_col: str = "hour",
    day_year_col: str = "day_year",
    window_days: int = 45,
    year_length: int = 366,
    quantiles=(0.1, 0.5, 0.9),
) -> pd.DataFrame:
    """
    Build local residual quantiles by (day_type, hour, day_year),
    using a circular +/- window_days neighborhood on day_year.
    """
    out = []

    for day_type in df[day_type_col].unique():
        for hour in sorted(df.loc[df[day_type_col] == day_type, hour_col].unique()):
            sub = df[(df[day_type_col] == day_type) & (df[hour_col] == hour)].copy()

            days = sub[day_year_col].to_numpy()
            residuals = sub[resid_col].to_numpy()
            ...
            for target_day in range(1, year_length + 1):
                ...
                mask = circ_dist <= window_days
                local_resid = residuals[mask]

                row = {
                    day_type_col: day_type,
                    hour_col: hour,
                    day_year_col: target_day,
                }


                vals = np.quantile(local_resid, quantiles)
...
def normalize_prices_by_season_year(df, season_map_number) -> pd.DataFrame:
    """
    Clean and normalize electricity prices by season of the year median.
    ...
    norm_season_year = df.groupby(["season", "year"])["Preis"].median()

    df["normalized_median"] = df.set_index(["season", "year"]).index.map(norm_season_year)
...
def fit_probabilistic_model(
    ...
    train = train.merge(
        sdh_effects[["day_type", "hour", "day_year", "sdh_effect"]],
        on=["day_type", "hour", "day_year"],
        how="left"
    )
    ...
    # 3) sinus
    X_sin = sm.add_constant(train[["sin_year", "cos_year"]], has_constant="add")
    sin_model = sm.OLS(train["resid_after_sdh"], X_sin).fit()
    ...
    # 4) local probabilistic layer
    residual_quantiles_local = build_local_residual_quantiles(
        df=train,
        resid_col="final_residual",
        day_type_col="day_type",
        hour_col="hour",
        day_year_col="day_year",
        window_days=window_days,
        year_length=year_length,
        quantiles=quantiles,
    )
    ...
    return {
        ...
        "sdh_effects": sdh_effects,
        "sin_model": sin_model,
        "residual_quantiles_local": residual_quantiles_local,
...
def predict_probabilistic(model, df_future: pd.DataFrame) -> pd.DataFrame:
    ...
    df_future["day_year"] = df_future["Datum"].dt.dayofyear
    daytype_effects = model["daytype_effects"]
    sdh_effects = model["sdh_effects"]
    sin_model = model["sin_model"]
    residual_quantiles_local = model["residual_quantiles_local"]
    #global_residual_quantiles = model["global_residual_quantiles"]
    quantiles = model["quantiles"]


    # deterministic part
    df_future["daytype_effect"] = df_future["day_type"].map(daytype_effects.to_dict())
    ...
    # probabilistic part: local residual quantiles
    rq = residual_quantiles_local.rename(columns={
        0.1: "q_10",
        0.5: "q_50",
        0.9: "q_90",
    })
    ...
    df_future = df_future.merge(
        rq[["day_type", "hour", "day_year", "q_10", "q_50", "q_90"]],
        on=["day_type", "hour", "day_year"],
        how="left"
    )
    ...
    # fallbacks
    #fallback_map = {
    #    "q_10": global_residual_quantiles.loc[0.1],
    #    "q_50": global_residual_quantiles.loc[0.5],
    #    "q_90": global_residual_quantiles.loc[0.9],
    #}

    #for q_name in ["q_10", "q_50", "q_90"]:
    #    df_future[q_name] = df_future[q_name].fillna(fallback_map[q_name])
...
def compute_season_shape(df):
    """
    Estimate seasonal shape ratios for inference.

    The model is trained on prices normalized by seasonal medians.
    To reintroduce seasonality in a consistent way, we compute
    seasonal factors expressed relative to the annual mean.

    Steps:
    1. Compute the median price for each (year, season)
    2. Compute the annual mean price for each year
    3. Compute the ratio median / annual mean
    4. Average these ratios across years to obtain stable seasonal factors

    Returns:
        shape_ratios: dict {season: seasonal_factor}
    """

    # --- Step 1: median per (year, season)
    season_median = (
    ...
    # --- Step 3: merge
    merged = season_median.merge(yearly_mean, on="year", how="left")

    # --- Step 4: compute ratio
    merged["ratio"] = merged["season_median"] / merged["yearly_mean"]

    # --- Step 5: average across years
    shape_ratios = (
        merged.groupby("season")["ratio"]
        .mean()
        .to_dict()
    )
