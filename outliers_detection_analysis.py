...
def recursive_filter_prices( df, price_col="residuals", col_output="filtered_rfp", col_tag = "is_outlier_rfp", max_iter=20, threshold=3):
    """
    Détecte les outliers par filtre récursif sur les prix.

    Règle:
        |P_t - mean| > threshold * std

    À chaque itération:
        - on recalcule mean/std sur les observations non détectées
        - on ajoute les nouveaux outliers
        - on s'arrête quand il n'y a plus de nouveaux outliers
...
def plot_filter_comparisons(
    ...
    """
    Compare visuellement deux méthodes de filtering :
    - standard filter
    ...
    3) KDE des distributions (original / STD / RFP)
    4) tableaux de synthèse si les colonnes de groupement existent

    Si save_png=True :
    - sauvegarde le graphe en PNG
    - sauvegarde le tableau summary en PNG
    """
